...
(function (global, factory) {
  ...
  var Nominatim = function Nominatim(base, els) {
    ...
    this.initializeLocation(this.options.initialLocation);
  };
  ...
  Nominatim.prototype.initializeLocation = function initializeLocation(city) {
    var this$1 = this;

    const provider = this$1.getProvider({
      query: city,
      provider: this.options.provider,
      key: this.options.key,
      lang: this.options.lang,
    ...
    json(ajax).then(function (res) {
      ...
      if (res_) {
        ...
        switch (this$1.options.provider) {
          case PROVIDERS.OSM:
            addressHtml =
              "<span class=\"" + (klasses$1.road) + "\">" + (res_[0].address.name) + "</span>";
            break;
          default:
            addressHtml = this$1.addressTemplate(res_[0].address);
        }

        this$1.chosen(res_[0], addressHtml, res_[0].address, res_[0].original);
      }
    }).catch(function (err) {
      removeClass(this$1.els.reset, klasses$1.spin);
      const li = createElement(
